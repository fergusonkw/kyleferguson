...
<title>Drafts awaiting approval</title>
</head>
    ...
    <table width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;width:100%;">
      ...
      <tr>
        <td style="background:#15161a;border:1px solid #2b2e36;padding:32px 36px 28px;">
          ...
          <h1 style="margin:0 0 10px;font-size:24px;font-weight:600;letter-spacing:-0.02em;color:#e9e9ec;line-height:1.2;">Still waiting<br>on approval.</h1>
          <p style="margin:0;font-size:15px;color:#71747e;line-height:1.6;max-width:440px;">
            {{ $drafts->count() }} draft{{ $drafts->count() === 1 ? '' : 's' }} {{ $drafts->count() === 1 ? 'has' : 'have' }}
            not been approved. Until {{ $drafts->count() === 1 ? 'it is' : 'they are' }}, this is work you have done and not billed for.
          </p>
        </td>
      </tr>
      ...
      <tr>
        <td style="background:#c0392b;padding:10px 36px;">
          <span style="font-family:monospace,monospace;font-size:11px;letter-spacing:0.1em;color:#fff;">{{ $money($total) }} UNBILLED</span>
        </td>
      </tr>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-bottom:1px solid #2b2e36;">
              <tr>
                <td style="padding:9px 0;font-size:14px;color:#e9e9ec;">
                  {{ $draft->client_snapshot['name'] ?? $draft->client->name }}
                  <span style="color:#4b4e57;font-family:monospace,monospace;font-size:11px;"> · {{ $draft->invoice_number }} · {{ $draft->created_at->diffForHumans() }}</span>
                </td>
                <td align="right" style="padding:9px 0;font-size:14px;color:#e9e9ec;">{{ $money($draft->total) }} {{ $draft->issue_currency }}</td>
              </tr>
            </table>
          @endforeach
        </td>
